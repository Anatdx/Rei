...
std::vector<uint32_t> get_allow_list(bool allow) {
    ...
    int ret = ksuctl(allow ? KSU_IOCTL_GET_ALLOW_LIST : KSU_IOCTL_GET_DENY_LIST, &cmd);
    if (ret < 0) {
        return {};
    }

    std::vector<uint32_t> out;
    out.reserve(cmd.count);
    for (uint32_t i = 0; i < cmd.count && i < 128; ++i) {
        out.push_back(cmd.uids[i]);
    }
    return out;
}

bool uid_granted_root(uint32_t uid) {
    UidGrantedRootCmd cmd{};
    cmd.uid = uid;
...
std::optional<AppProfile> get_app_profile(uint32_t uid, const std::string& key) {
    GetAppProfileCmd cmd{};
    memset(&cmd, 0, sizeof(cmd));
    cmd.profile.version = KSU_APP_PROFILE_VER;
    cmd.profile.current_uid = static_cast<int32_t>(uid);
    if (!key.empty()) {
        strncpy(cmd.profile.key, key.c_str(), KSU_MAX_PACKAGE_NAME - 1);
    }
...
int set_app_profile(const AppProfile& profile) {
    SetAppProfileCmd cmd{};
    memset(&cmd, 0, sizeof(cmd));
    cmd.profile = profile;
    return ksuctl(KSU_IOCTL_SET_APP_PROFILE, &cmd);
}
