...
NativeBridge(setAppProfile, jboolean, jobject profile) {
  ...
  const char *cpkg =
      GetEnvironment()->GetStringUTFChars(env, (jstring)key, NULL);
  ...
  if (allowSu) {
    ...
    const char *cdomain =
        GetEnvironment()->GetStringUTFChars(env, (jstring)domain, NULL);
...
NativeBridge(authenticateSuperKey, jboolean, jstring superKey) {
  ...
  const char *cSuperKey =
      GetEnvironment()->GetStringUTFChars(env, superKey, NULL);
